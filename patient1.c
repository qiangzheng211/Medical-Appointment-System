...
int main()
{
    ...
    fgets(patient1str,MAXDATASIZE,fp1);
    patient1str[20]='\0';
    
    /* opening patient1insurance.txt */
    ...
    portstr[5] = '\0';
    printf("\nPhase 1: Patient1 has TCP port number %s and IP address %s.\n", portstr,localip);
    
    /* Authentication */
    authstr = concat("authenticate ", patient1str);
    send(sockfd,authstr,strlen(authstr), 0);
    username = strtok(patient1str, s);
    ...
    printf("\nPhase 1: End of Phase 1 for patient1.\n");
    ...
    printf("\nPhase 2: The following appointments are available for patient1: \n%s", avai_data);
    ...
    printf("\nPhase 2: The requested appointment from Patient 1 is available and reserved to Patient 1. The assigned doctor number is %s.\n", docport);
    close(sockfd);
    ...
    sendto(udpsockfd, "patient1", strlen("patient1"), 0, udpservinfo->ai_addr, udpservinfo->ai_addrlen);
    ...
    printf("\nPhase 3: Patient 1 has a dynamic UDP port number %s and IP address %s.\n", udpport, localip);
    ...
    {
        printf("\nPhase 3: Patient 1 has received %s$ estimation cost from doctor with portnumber %s and name doc1.\n", esticost, docport);
    }
    else {
        printf("\nPhase 3: Patient 1 has received %s$ estimation cost from doctor with portnumber %s and name doc2.\n", esticost, docport);
         }
